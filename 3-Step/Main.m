...
for idx = 1:numSteps-1
    ...
    elseif idx == 2
        ...
        ddq(:,idx) = (3*dq(:,idx) - 4*dq(:,idx-1) + dq(:,idx-2)) / (2*step);
    end
    ...
    if idx < 3
        ...
        q(:,idx+1) = 3/2*q(:,idx) - q(:,idx-1) + 1/2*q(:,idx-2) + step * dq(:,idx);
        JHat(:,:,idx+1) = 3/2* JHat(:,:,idx) - JHat(:,:,idx-1) + 1/2* JHat(:,:,idx-2) + step * dJHat(:,:,idx);
    end
    idx
end
...
save (['data/3step',num2str(eta),'te-2'], 't', 'zeta', 'eta', 'iota', 'Td', 'd','L0','step','initPos','q','dq','JHat','xd','xa');